...
export default function DashboardPage() {
  ...
  if (authLoading || loading) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center text-muted">Loading dashboard...</div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatCard icon={Code2} label="Solved" value={displayUser.solvedCount || 0} />
        <StatCard icon={TrendingUp} label="Rating" value={displayUser.rating || 0} suffix={rating.tier} />
        <StatCard icon={Flame} label="Streak" value={0} suffix="days" />
        <StatCard icon={Trophy} label="Global Rank" value={`#${(displayUser.rank || 0).toLocaleString()}`} />
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            {recentSubmissions.length > 0 ? (
              ...
              <p className="text-sm text-muted">No submissions yet. <Link href="/practice" className="text-primary">Start solving</Link></p>
            )}
          </Card>
        </div>
                ...
                {topicProgress.map((topic) => (
                  <div key={topic.topic}>
                    <div className="flex items-center justify-between text-sm mb-1.5">
                      ...
                      <span className="text-muted text-xs">{topic.solved}/{topic.total}</span>
                    </div>
                    <div className="h-2 bg-background rounded-full overflow-hidden">
                      <div className="h-full bg-primary rounded-full" style={{ width: `${topic.percentage}%` }} />
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-muted">Solve problems to track topic progress.</p>
            )}
          </Card>
                ...
                {duelHistory.slice(0, 3).map((duel) => (
                  <div key={duel.id} className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted">{formatDate(duel.date)}</p>
                    </div>
